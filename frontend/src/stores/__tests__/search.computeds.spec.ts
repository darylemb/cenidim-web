...
describe('search store computeds', () => {
  ...
  it('hasResults is true when results have length', async () => {
    vi.mocked(apiService.searchSongs).mockResolvedValue({
      results: [{ id: 1, fonograma_id: 1, title: 'A', filename: '', lyrics: '',
  ...
  it('isLoading reflects the loading state', async () => {
    ...
    vi.mocked(apiService.searchSongs).mockImplementation(() => new Promise(r => {
      resolvePromise = r
    }))
    const s = useSearchStore()
    ...
    vi.mocked(apiService.searchSongs).mockResolvedValue({
      results: [
        { id: 1, fonograma_id: 1, title: 'A', filename: '', lyrics: '', album: '',
          ...
          ciudad_edicion: '', pais_edicion: '', year: '', pistas: '', observaciones: '',
          clasificacion: '', tema: '' },
        { id: 2, fonograma_id: 1, title: 'B', filename: '', lyrics: '', album: '',
          subtitulo: '', interprete_principal: '', interpretes_invitados: '',
          interprete_participante: '', soporte_fisico: '', editora: '', numero_catalogo: '',
          ciudad_edicion: '', pais_edicion: '', year: '', pistas: '', observaciones: '',
          clasificacion: '', tema: '' },
