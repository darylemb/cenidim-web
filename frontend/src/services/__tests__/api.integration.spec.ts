...
describe('api.ts fetch integration', () => {
  ...
  it('parses a 2xx JSON response and returns the body', async () => {
    const fetchMock = vi.fn(
      async () =>
        new Response(
          '{"token":"abc","user":{"id":1,"username":"u","email":"u@x","role":"viewer"}}',
          {
            status: 200,
    ...
    const { apiService } = await import(/* @vite-ignore */ '../api?integration=1');
  ...
  it('throws with the server error message on 4xx', async () => {
    vi.stubGlobal(
      'fetch',
      vi.fn(async () => new Response('{"error":"No autorizado"}', { status: 401 }))
    );
    const { apiService } = await import(/* @vite-ignore */ '../api?integration=2');
  ...
  it('falls back to "Login failed" when 5xx has no error field', async () => {
    ...
    const { apiService } = await import(/* @vite-ignore */ '../api?integration=3');
  ...
  it('attaches Authorization header when a token is in localStorage', async () => {
    ...
    const fetchMock = vi.fn(
      async () =>
        new Response('{"id":1,"username":"u","email":"u@x","role":"admin","created_at":""}', {
          status: 200,
        })
    );
    vi.stubGlobal('fetch', fetchMock);
    const { apiService } = await import(/* @vite-ignore */ '../api?integration=4');
  ...
  it('does not attach Authorization when there is no token', async () => {
    ...
    vi.stubGlobal('fetch', fetchMock);
    const { apiService } = await import(/* @vite-ignore */ '../api?integration=5');
  ...
  it('encodes query params for getStats', async () => {
    ...
    vi.stubGlobal('fetch', fetchMock);
    const { apiService } = await import(/* @vite-ignore */ '../api?integration=6');
  ...
  it('encodes search options correctly', async () => {
    const fetchMock = vi.fn(async () => new Response('{}'));
    vi.stubGlobal('fetch', fetchMock);
    const { apiService } = await import(/* @vite-ignore */ '../api?integration=7');
  ...
  it('serializes admin PUT with the right method + body', async () => {
    ...
    const fetchMock = vi.fn(async () => new Response('{}'));
    vi.stubGlobal('fetch', fetchMock);
    const { apiService } = await import(/* @vite-ignore */ '../api?integration=8');
  ...
  it('forgotPassword posts just the email', async () => {
    ...
    vi.stubGlobal('fetch', fetchMock);
    const { apiService } = await import(/* @vite-ignore */ '../api?integration=9');
  ...
  it('resetPassword posts token + new_password', async () => {
    const fetchMock = vi.fn(async () => new Response('{"ok":true}', { status: 200 }));
    vi.stubGlobal('fetch', fetchMock);
    const { apiService } = await import(/* @vite-ignore */ '../api?integration=10');
  ...
  it('admin endpoints wrap DELETE with no body', async () => {
    ...
    const fetchMock = vi.fn(async () => new Response('{}', { status: 200 }));
    vi.stubGlobal('fetch', fetchMock);
    const { apiService } = await import(/* @vite-ignore */ '../api?integration=11');
  ...
  it('adminUpdateUser sends whatever payload it receives (caller strips empty pw)', async () => {
    ...
    const fetchMock = vi.fn(async () => new Response('{}'));
    vi.stubGlobal('fetch', fetchMock);
    const { apiService } = await import(/* @vite-ignore */ '../api?integration=12');
  ...
  it('adminUpdateUser keeps the password when non-empty', async () => {
    ...
    const fetchMock = vi.fn(async () => new Response('{}'));
    vi.stubGlobal('fetch', fetchMock);
    const { apiService } = await import(/* @vite-ignore */ '../api?integration=13');
    await apiService.adminUpdateUser(7, {
      id: 7,
      username: 'bob',
      email: 'b@x',
  ...
  it('non-2xx with empty body falls back to the operation name', async () => {
    ...
    const { apiService } = await import(/* @vite-ignore */ '../api?integration=14');
    await expect(apiService.login('u', 'p')).rejects.toThrow('Login failed');
  });
  ...
  it('adminGetFonograma fetches by id', async () => {
    const fetchMock = vi.fn(async () => new Response('{"clave_fonograma":1,"titulo":"A"}'));
    vi.stubGlobal('fetch', fetchMock);
  ...
  it('adminCreateSong posts the payload', async () => {
    const fetchMock = vi.fn(async () => new Response('{"id":99,"title":"X"}'));
    vi.stubGlobal('fetch', fetchMock);
  ...
  it('adminUpdateSong puts to /admin/songs/{id}', async () => {
    const fetchMock = vi.fn(async () => new Response('{"message":"Song updated"}'));
    vi.stubGlobal('fetch', fetchMock);
  ...
  it('adminDeleteSong issues a DELETE', async () => {
    const fetchMock = vi.fn(
      async () => new Response('{"message":"Song deleted"}', { status: 200 })
    );
    vi.stubGlobal('fetch', fetchMock);
  ...
  it('adminCreateUser posts username + email + password + role', async () => {
    const fetchMock = vi.fn(
      async () => new Response('{"user":{"id":1,"username":"u","email":"u@x","role":"viewer"}}')
    );
    vi.stubGlobal('fetch', fetchMock);
  ...
  it('adminDeleteUser issues a DELETE', async () => {
    localStorage.setItem('cenidim_token', 'TKN');
    const fetchMock = vi.fn(
      async () => new Response('{"message":"User deleted"}', { status: 200 })
    );
    vi.stubGlobal('fetch', fetchMock);
  ...
  it('register posts the body and returns the response', async () => {
    const fetchMock = vi.fn(
      async () =>
        new Response(
          '{"token":"reg-tok","user":{"id":1,"username":"alice","email":"a@x","role":"viewer"}}'
        )
    );
    vi.stubGlobal('fetch', fetchMock);
  ...
  it('register throws on 4xx with the server error message', async () => {
    vi.stubGlobal(
      'fetch',
      vi.fn(async () => new Response('{"error":"email ya existe"}', { status: 409 }))
    );
  ...
  it('adminCreateFonograma returns the new row', async () => {
    const fetchMock = vi.fn(
      async () => new Response('{"clave_fonograma":99,"titulo":"New"}', { status: 201 })
    );
    vi.stubGlobal('fetch', fetchMock);
  ...
  it('adminCreateFonograma throws on conflict', async () => {
    vi.stubGlobal(
      'fetch',
      vi.fn(async () => new Response('{"error":"ClaveFonograma already exists"}', { status: 409 }))
    );
    const { apiService } = await import('../api?integration=g16');
    await expect(apiService.adminCreateFonograma({ clave_fonograma: 1 })).rejects.toThrow(
      'ClaveFonograma already exists'
    );
  });
  ...
  it('searchSongs throws when the server returns 500', async () => {
    vi.stubGlobal(
      'fetch',
      vi.fn(async () => new Response('{"error":"Database error"}', { status: 500 }))
    );
  ...
  it('getWordCloud surfaces non-2xx with the generic HTTP error', async () => {
    vi.stubGlobal(
      'fetch',
      vi.fn(async () => new Response('{"error":"bad"}', { status: 500 }))
    );
