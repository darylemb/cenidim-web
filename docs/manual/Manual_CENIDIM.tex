...
\begin{titlepage}
  ...
  \begin{center}
    ...
    \begin{tabular}{rl}
      ...
      \textbf{Rama de referencia:}    & \codigo{sandbox} \\
...
(\codigo{backend-fastapi/app/models/theme_normalization.py}).
...
\codigo{backend-fastapi/openapi.json} (OpenAPI 3.1, regenerable con
...
la rama \codigo{sandbox} en la fecha de este documento:
...
\begin{table}[h]
  ...
  \begin{tabular}{lll}
    ...
    \midrule
    Tests backend   & \codigo{pytest tests/}        & \ok\ 235 pasan, cob. 96.13\% \\
    ...
    Tipos backend   & \codigo{mypy app/}             & \pend\ 41 errores (ver \S\ref{cap:pendientes}) \\
    \bottomrule
  \end{tabular}
  \caption{Panel de calidad. $^{\ast}$Requieren el ajuste de
    \codigo{localStorage} descrito en el capítulo~\ref{cap:pendientes}.}
  \label{tab:calidad}
\end{table}
...
\begin{itemize}
  \item Cobertura backend $\ge 80\%$ (\codigo{--cov-fail-under},
        \codigo{pyproject.toml}); cobertura actual 96.13\%.
  ...
  \item Humo post-despliegue:
        \codigo{backend-fastapi/scripts/smoke.sh} (código de salida no
...
\section{Veredicto}
El sistema es \textbf{funcionalmente completo y está desplegado} en
producción (\codigo{cenidim.\allowbreak darylemb.dev}). La suite de backend está
íntegramente en verde y la de frontend también, una vez resuelto el
conflicto de entorno descrito abajo. No obstante, la entrega formal
debería ir precedida de la limpieza que se lista a continuación.
...
\begin{longtable}{p{0.9cm}p{3.4cm}p{9.4cm}}
  ...
  \endhead
  \pend & Tipado backend &
    \codigo{mypy app/} reporta \textbf{41 errores} en 7 archivos
    (p.\,ej. \codigo{public.py}, \codigo{admin.py}, \codigo{main.py}).
    El hook de pre-commit ejecuta mypy, por lo que bloquea commits;
    conviene corregirlos o ajustar la configuración de forma
    deliberada. \\
  \pend & Tests frontend &
    Con Node $\ge$ 24, el \codigo{localStorage} nativo sombrea al de
    jsdom y 63 pruebas fallan
    (\codigo{Cannot read properties of undefined}). Solución:
    fijar \codigo{globalThis.localStorage} en
    \codigo{src/test/setup.ts} o exportar
    \codigo{NODE\_OPTIONS=\allowbreak --localstorage-file=<tmp>} en CI. Verificado:
    con el ajuste pasan los 265 tests. \\
  \pend & Dependencias frontend &
    \codigo{npm audit}: \textbf{15 vulnerabilidades} (1 crítica,
    10 altas, incl. \codigo{ws} en \codigo{vue-wordcloud}). Revisar
    \codigo{npm audit fix} o sustituir el componente de nube. \\
  \pend & Higiene del repo &
    La limpieza de \codigo{ff4303b} ya retiró \codigo{test-auth.js},
    \codigo{untitled41.py}, \codigo{datos.html}, \codigo{index.html} y
    \codigo{plans/}. Quedan: \codigo{Clasificacion.csv} y
    \codigo{clasification.csv} (ninguno es referenciado por el pipeline,
    que usa \codigo{db_fonografia.csv}) y dos \codigo{letras.db}
    divergentes trackeados (\codigo{/letras.db} y
    \codigo{/data/letras.db}). \\
  \pend & Documentación &
    La misma limpieza eliminó \emph{toda} la documentación
    (\codigo{docs/}: ADR, CUTOVER, GLOSARIO, respuestas al cliente) más
    \codigo{AGENTS.md} y \codigo{CHANGELOG.md}; si fue intencional,
    conviene decidir si se restauran o se acepta su pérdida. Los README
    fueron corregidos en la rama \codigo{release} para no referenciar
    archivos inexistentes. Sigue sin existir un archivo
    \codigo{LICENSE}; \codigo{.gitignore} lista
    \codigo{docker-compose.yaml} aunque está trackeado. \\
  \pend & Observabilidad &
    Deprecación FastAPI: \codigo{on_event} en \codigo{app/main.py}
    debe migrarse a manejadores \emph{lifespan}. \\
  \bottomrule
  \caption{Pendientes técnicos detectados en la revisión.}
...
\bibitem{readme} Repositorio del proyecto,
  \textit{README.md} (raíz) y \textit{backend-fastapi/README.md}.
...
\bibitem{openapi} Especificación OpenAPI 3.1 de la API,
  \codigo{backend-fastapi/openapi.json}.
