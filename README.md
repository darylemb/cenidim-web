...
5. **Historical material**: `historical/` holds the pre-automation manual-classification worksheets (`Clasificacion.csv`, `clasification.csv`) plus a provenance README. They are kept as editorial reference only — **they are not pipeline inputs** (the classification in `letras.db` is computed by spaCy in `classify_songs.py`). The tracked `letras.db` copies (repo root and `data/`) are likewise kept as a maintainer reference; both are regenerated by `db-init` / `build_db.sh` on every stack boot.
...
- **Backend (FastAPI)**: `cd backend && PYTHONPATH=. uv run pytest tests/`. **235 tests pass at 83% coverage** (single clean-run figure; the 80% gate is enforced via `--cov-fail-under=80` in `pyproject.toml`). `ruff check` and `mypy app/` are both clean.
- **Frontend**: `cd frontend && npm run test -- --run` (Vitest 4 + Vue Test Utils). **265 tests pass** (9 skipped). Strict TypeScript — the typecheck (`vue-tsc --noEmit`) runs inside `npm run build` in CI.
- **Dependencies**: `npm audit` reports **0 vulnerabilities** (the unused `vue-wordcloud` dependency was removed and vitest upgraded to 4.1.11).
